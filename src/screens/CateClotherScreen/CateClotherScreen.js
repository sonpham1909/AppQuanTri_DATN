...
const CateClotherScreen = ({route}) => {
  ...
  // Hàm tải sản phẩm và biến thể (variants) theo danh mục con
    ...
    setFilterState(prev => {
      ...
      return {
        ...
        [index]: updatedFilters,
         // Cập nhật bộ lọc cho tab hiện tại
      };
  ...
  const renderScene = ({ route }) => {
    ...
    // Ensure `filteredProductsState` is a valid array or set to null
    const tabProducts = Array.isArray(filteredProductsState?.[routeIndex])
      ? filteredProductsState[routeIndex]
      : null;
    ...
    if (tabProducts === null) {
      // Handle case where no products exist in the selected subcategory
      return (
        <View style={styles.container}>
         <View style={styles.container}>
           <ActivityIndicator size="large" color="#00A65E" />
    ...
    if (tabProducts.length === 0) {
      // Handle case where no products match the filters
      return (
        <View style={styles.container}>
    ...
    // Render the product list if products are available
    return (
      <View style={styles.container}>
        <ProductList navigation={navigation} products={tabProducts} />
      </View>
    );
  };
...
//   // Hàm load sản phẩm và biến thể (variants) chỉ khi dữ liệu chưa có
//   const loadProductsAndVariants = async () => {
//     if (filteredProductsState[index]) {
//       return; // Không tải lại nếu dữ liệu đã có
//     }

//     try {
//       setLoading(true);
//       const productsResponse = await dispatch(fetchProductsBySubCategory(currentSubCategoryId));
//       await dispatch(fetchColorsAndSizesBySubCategoryId(currentSubCategoryId));
...
//   // Hàm lọc sản phẩm khi bộ lọc thay đổi
//   const filterProducts = async () => {
...
//       setNoProductsFound(response.payload.length === 0);
//     } catch (error) {
//       console.error('Error filtering products:', error);
//     } finally {
//       setLoading(false);
...
//   // useEffect: Chỉ tải sản phẩm lần đầu tiên và khi thay đổi tab
//   useEffect(() => {
//     if (currentSubCategoryId && !filteredProductsState[index]) {
...
//   // useEffect: Chỉ lọc lại khi bộ lọc thay đổi
//   useEffect(() => {
//     if (filterState[index]) {
//       filterProducts();
//     }
...
//           <Text>{filterState[index]?.size?.length > 0 ? renderTextWithEllipsis(filterState[index].size) : 'Kích cỡ'}</Text>
//           <MaterialCommunityIcons name="chevron-down" size={18} color="#666" />
//         </TouchableOpacity>

//         <TouchableOpacity style={styles.filterButton} onPress={() => setColorFilterVisible(true)}>
//           <Text>{filterState[index]?.color?.length > 0 ? renderTextWithEllipsis(filterState[index].color) : 'Màu sắc'}</Text>
//           <MaterialCommunityIcons name="chevron-down" size={18} color="#666" />
//         </TouchableOpacity>

//         <TouchableOpacity style={styles.filterButton1} onPress={() => setPriceFilterVisible(true)}>
//           <Text>{filterState[index]?.minPrice != null && filterState[index]?.maxPrice != null ? `Giá: ${filterState[index].minPrice.toLocaleString('vi-VN')} - ${filterState[index].maxPrice.toLocaleString('vi-VN')} VND` : 'Giá'}</Text>
//           <MaterialCommunityIcons name="chevron-down" size={18} color="#666" />
//         </TouchableOpacity>
//       </ScrollView>
//     </View>
...
//         <View style={styles.container}>
//           <Text style={{textAlign: 'center', fontSize: 16, color: '#666'}}>
//             Không có sản phẩm trong danh mục con.
//           </Text>
//         </View>
//       );
//     }

//    else if (tabProducts.length === 0) {
//       return (
//         <View style={styles.container}>
//           <Text style={{textAlign: 'center', fontSize: 16, color: '#666'}}>
//             Không có sản phẩm phù hợp với bộ lọc.
...
//       );
//     } else {
//       // Khi có sản phẩm, hiển thị danh sách sản phẩm
//       return (
//         <View style={styles.container}>
//           <ProductList navigation={navigation} products={tabProducts} />
//         </View>
...
//         onClose={() => setSizeFilterVisible(false)}
//         applyFilters={selectedSizes => applyFilterState({size: selectedSizes})}
//         initialFilters={filterState[index]?.size}
//       />
//       <ColorFilterModal
//         visible={colorFilterVisible}
...
//         onClose={() => setColorFilterVisible(false)}
//         applyFilters={selectedColors => applyFilterState({color: selectedColors})}
//         initialFilters={filterState[index]?.color}
//       />
//       <PriceFilterModal
//         visible={priceFilterVisible}
